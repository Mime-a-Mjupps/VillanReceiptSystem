// Description: Node.JS module for printing to Villans thermal printers.
...
class Printer {
  ...
  constructor(netPath, type) {
    ...
    this.printer = new ThermalPrinter({
      type: this.printerType, // Printer type: 'star' or 'epson'
      interface: this.netPath, // Printer netPath
      characterSet: "ISO8859_15_LATIN9", // Printer character set - ISO8859_15_LATIN9
  ...
  async genPicOrderId(id) {
    ...
    let yourOrder = "Your order number is";
   ...
   */
  async printOrderCustomer(orderId, itemsInOrder, meme) {
    ...
    try {
      ...
      await this.printer.printImageBuffer(await this.genPicOrderId(orderId));
      if(meme){
        this.printer.println(meme)
      }
      this.printer.cut();
